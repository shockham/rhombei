...
float scene(vec3 p) {
	...
	float o_octa = max(
		-box(
            q + vec3(0.0, -0.7 - sin(time.x / 2.6), 0.0),
            vec3(2.0, 0.5, 2.0)
        ),
		onion(onion(onion(
			octa(
...
void main() {
    ...
    vec3 cam_pos = vec3(
        (cos(input_cam_pos.x) * cos(input_cam_pos.y)) + cos(time.x / 8.5),
        input_cam_pos.y - 0.25 + (sin(time.x / 3.5) * 0.25),
        (sin(input_cam_pos.x) * cos(input_cam_pos.y)) + sin(time.x / 12.2)
    ) * distance;
